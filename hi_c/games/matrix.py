...
class MatrixGame:
    """Differentiable model of a general sum 2x2 matrix game"""
    ...
    def __init__(self,
        ...
        assert payoffs.shape == (2, 2, 2), "must specify a two-player 2x2 matrix game"

        self._payoffs = torch.tensor(payoffs, dtype=torch.float, device=device)
        self.strategy_spaces = [
            Box(-init_range, init_range, (1,)),
            Box(-init_range, init_range, (1,))
        ]
    ...
    def payoffs(self, params_a, params_b):
        ...
        s_a = torch.sigmoid(params_a)
        s_b = torch.sigmoid(params_b)
        strategy_a = torch.stack([s_a, 1. - s_a], dim=-1)
        strategy_b = torch.stack([s_b, 1. - s_b], dim=-1)
        ...
        if self._entropy > 0.:
            entropy_a = s_a * torch.log(s_a) + (1. - s_a) * torch.log(1. - s_a)
            entropy_b = s_b * torch.log(s_b) + (1. - s_b) * torch.log(1. - s_b)
...
class IteratedMatrixGame:
    ...
    def payoffs(self, params_a, params_b):
        ...
        if self._entropy > 0.:
            entropy_a = strategy_a * torch.log(strategy_a) + (1. - strategy_a) * torch.log(1. - strategy_a)
            entropy_b = strategy_b * torch.log(strategy_b) + (1. - strategy_b) * torch.log(1. - strategy_b)
            return_a = return_a - self._entropy * torch.sum(entropy_a, dim=-1)
            return_b = return_b - self._entropy * torch.sum(entropy_b, dim=-1)
